...
function gameEngine(state, move) {
  ...
  return new Promise((resolve, reject) => {
    ...
    switch (move.card) {
      ...
      case BARON: // Compare the cards of the originating player and selected player
        ...
        for (let i = 0; i < newState.players.order.length; i++) {
          const id = newState.players.order[i].toString();
          if (id.toString() !== move.player.toString()) {
            console.log(`Not the current player, current player is:`, move.player,
              `id is:`, id);
            opponents.push(id);
          }
        }
        if (newState.players[move.player] === newState.players[move.chosenPlayer]) {
          console.log(`length:`, newState.players.order.length,
            'opponents are:', opponents, `first opponent has:`,
            newState.players[opponents[0]].discarded[0], `second opponent has`, newState.players[opponents[1]].discarded[0]);
          if ((newState.players.order.length === 3
            && newState.players[opponents[0]].discarded[0] === 4
            && newState.players[opponents[1]].discarded[0] === 4)
            || (newState.players.order.length === 2
            && newState.players[opponents[0]].discarded[0] === 4)) {
            // ourCard = 0;
            // theirCard = 0;
    ...
    // if chosen player is eliminated, then discard their hand
    if (newState.players[move.chosenPlayer]
      && newState.players[move.chosenPlayer].eliminated) {
      console.log(`Discard eliminated player's hand`);
      newState.players[move.player].discarded.unshift(
        newState.players[move.chosenPlayer].hand
      );
    }
    ...
    // remove eliminated players from playerOrder array
