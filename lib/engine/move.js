...
function gameEngine(state, move) {
  return new Promise((resolve, reject) => {
    ...
    let allPlayersProtected = (function() {
      for (let player in state.playerOrder) {
        if (state.players[state.playerOrder[player]].discarded[0] !== HANDMAID)
          return false;
      }
      return true;
    }());
    ...
    let nextPlayer = newState.playerOrder.indexOf(move.player.toString()) + 1;
